...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QLabel, QCheckBox, QSlider
)
...
class PLCControlView(QWidget):
    ...
    def _create_digital_outputs(self) -> QGroupBox:
        ...
        for signal_name, label_text in do_signals:
            ...
            cb.stateChanged.connect(lambda checked, sig=signal_name: self._toggle_output(sig, checked))
    ...
    def _create_analog_outputs(self) -> QGroupBox:
        ...
        for signal_name, label_text in ao_signals:
            ...
            slider.setValue(50 if "Motor" in label_text or "Flow" in label_text else 0)  # Default 50% for motor and flow
            slider.valueChanged.connect(lambda value, sig=signal_name, lbl=label: self._update_analog_output(sig, value, lbl))
