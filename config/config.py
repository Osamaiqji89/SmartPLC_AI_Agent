...
def merge_configs() -> AppConfig:
    ...
    if "openai" in yaml_config:
        ...
        config.openai_temperature = yaml_config["openai"].get(
    ...
    if "rag" in yaml_config:
        ...
        config.vector_store_dir = yaml_config["rag"].get(
            "vector_store_dir", config.vector_store_dir
        )

    if "plc" in yaml_config:
        config.plc_update_interval_ms = yaml_config["plc"].get(
            "update_interval_ms", config.plc_update_interval_ms
        )
        config.enable_simulation = yaml_config["plc"].get(
