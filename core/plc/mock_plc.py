...
class MockPLC:
    ...
    def _load_signals_from_db(self):
        ...
        try:
            ...
            for db_signal in db_signals:
                ...
                if not signal_type:
                    logger.warning(
                        f"Unknown signal type '{db_signal.type}' for {db_signal.name}, defaulting to DI"
                    )
    ...
    def _log_alarm(
        ...
        try:
            ...
            existing = (
                session.query(AlarmLog)
                .filter_by(signal_name=signal_name, acknowledged=False)
                .first()
            )
    ...
    def _auto_acknowledge_alarm(self, signal_name: str):
        ...
        try:
            ...
            alarms = (
                session.query(AlarmLog).filter_by(signal_name=signal_name, acknowledged=False).all()
            )
                ...
                for _signal_name, signal in self.signals.items():
                    ...
                    if db_signal:
                        db_signal.current_value = (
                            ...
                            else (1.0 if signal.value else 0.0)
                        )
    ...
    def _simulate_tank_process(self):
        ...
        try:
            ...
            flow_control = (
                self.read_signal("AO_01_FlowControl")
                if "AO_01_FlowControl" in self.signals
                else 50.0
            )
        ...
        try:
            ...
            if motor_running:
                ...
                motor_speed_setpoint = (
                    self.read_signal("AO_02_MotorSpeed")
                    if "AO_02_MotorSpeed" in self.signals
                    else 100.0
                )
